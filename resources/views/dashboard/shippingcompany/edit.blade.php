        ...
        <div class="card">
            <div class="card-body">
                ...
                <form class="parsley-style-1"
                    ...
                    autocomplete="off"
                    name="selectForm2"
                    action="{{route('admin.shippingcompany.update', $shippingcompany->id)}}"
                    method="POST"
                    >
                        ...
                        <div class="col-4">
                            <div class="form-group">
                                ...
                                <select name="fast" class="form-control">
                                    <option value="1" {{ old('fast',$shippingcompany->fast) == '1' ? 'selected' : null }}>Yes</option>
                                    <option value="0" {{ old('fast',$shippingcompany->fast) == '0' ? 'selected' : null }}>No</option>
                                </select>
                        ...
                        <div class="col-4">
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control">
                                    <option value="1" {{ old('status',$shippingcompany->status) == '1' ? 'selected' : null }}>Active</option>
                                    <option value="0" {{ old('status',$shippingcompany->status) == '0' ? 'selected' : null }}>Inactive</option>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="countries[]" class="form-control select-multiple-tags" multiple="multiple">
                                    @forelse($countries as $country)
                                        <option value="{{ $country->id }}" {{ in_array($country->id, old('countries', $shippingcompany->country->pluck('id')->toArray() ) )? 'selected' : null }} >{{ $country->name }}</option>
                                    @empty
                                    @endforelse
                                </select>
